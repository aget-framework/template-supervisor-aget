...
## Project Context
template-supervisor-aget - Supervisor AGET template - v2.7.0
...
```json
{
  "portfolio": "main"  // or "example", "legalon", null (for supervisor itself)
}
```
...
**Creation Process**:
1. Copy from appropriate template at v2.7.0+:
...
**Template Locations**:
- `aget-framework/template-worker-aget/` - Public worker agent template (v2.7.0)
- `aget-framework/template-advisor-aget/` - Advisory with internal state template (v2.7.0)
- `aget-framework/template-supervisor-aget/` - Supervisor template (v2.7.0)
...
---
*Generated by AGET v2.7.0 - https://github.com/aget-framework/template-supervisor-aget*
